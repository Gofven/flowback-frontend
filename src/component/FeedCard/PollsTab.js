...
export default function PollsTab(props) {
    ...
    return (
        <div className="tab-pane fade show active " id="PollsTab">
                ...
                {polls?.map((poll, index) => {
                    if (
                        ...
                        return <div className="poll-post" key={poll.id + poll.type + poll.discussion}>
                            <Link
                                to={`/groupdetails/${(poll && poll.group && poll.group.id) ? poll.group.id : groupId}/polldetails/${poll.id}`}>
            ...
            {
                (page < totalPage + 1) &&
                <div className="d-flex justify-content-end cursor-pointer" onClick={() => {
                    getPolls(false)
                }}>Load more... </div>
            }
        </div>
