...
export default function Prediction({ prediction }) {
    ...
    return <div key={prediction.id} className="p-3 m-4 bg-light rounded-3 shadow-xl">
        <Loader loading={loading}>
            ...
            <form>
                <div className="w-100">
                    <div className="d-flex">0<input className="w-100" type="range" id="prediction-vote-slider" max="100" min="0" value={score} onChange={scoreChange}></input>100</div>
                    <div className="text-center" style={{ "color": hasUnsavedChanges ? "#994422" : "black" }}>{score}%</div>
                </div>
